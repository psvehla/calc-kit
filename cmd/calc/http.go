...
import (
	...
	calckitsvr "calc-kit/gen/http/calc/kitserver"
	calcsvr "calc-kit/gen/http/calc/server"
	"context"
	"fmt"
	"net/http"
	"net/url"
...
func handleHTTPServer(ctx context.Context, u *url.URL, calcEndpoints *calc.Endpoints, wg *sync.WaitGroup, errc chan error, logger log.Logger, debug bool) {
	...
	// responses.
	var (
		calcMultiplyHandler *kithttp.Server
		calcServer          *calcsvr.Server
	)
	{
		eh := errorHandler(logger)
		calcMultiplyHandler = kithttp.NewServer(
			endpoint.Endpoint(calcEndpoints.Multiply),
			calckitsvr.DecodeMultiplyRequest(mux, dec),
			calckitsvr.EncodeMultiplyResponse(enc),
		)
		calcServer = calcsvr.New(calcEndpoints, mux, dec, enc, eh, nil, nil)
	}

	// Configure the mux.
	calckitsvr.MountMultiplyHandler(mux, calcMultiplyHandler)
	calckitsvr.MountGenHTTPOpenapiJSON(mux)
